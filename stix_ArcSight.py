...
def get_parser():
	...
	parser.add_option('-o', '--outfile', help='Output the data to a text file', action='store_true')
	...
	parser.add_option('-a', '--arcsight', help='Send the data to ArcSight using CEF Syslog', action='store_true')	
	...
	parser.add_option("--taxii_end_time", dest="end_ts", default=None, help="The end timestamp (YYYY-MM-dd HH:MM:SS) in UTC " +
		   "for the taxii poll request. Defaults to None.")
		...
		for item in values:
			try:
				...
				if re.match("^(http|https)", item):
					...
					if args[0].arcsight:
						cef = 'CEF:0|IHC-OSINT|Soltra|1.0|100|Known Malicious '+'Website'+'|1|request='+item+' shost='+u.netloc+' msg=NH-ISAC Malicious Domain ' + u.netloc
						time.sleep(.02)
					...
					if args[0].arcsight:
						cef = 'CEF:0|IHC-OSINT|Soltra|1.0|100|Known Malicious '+'Email'+'|1|suser='+item+' msg=NH-ISAC Malicious Email '+item
						time.sleep(.02)
						syslog(cef, host=dest,port=dest_port)
					...
					if args[0].outfile:
						with open('MyIPs.txt', 'a') as myips:
                                                        myips.writelines("%s\n" % item)
					if args[0].arcsight:
						cef = 'CEF:0|IHC-OSINT|Soltra|1.0|100|Known Malicious '+'Host'+'|1|src='+item+' msg=NH-ISAC Malicious IP '+item			
						time.sleep(0.2)
						syslog(cef, host=dest,port=dest_port)
				elif re.match("^:", item):
					if args[0].verbose:
					...
					if args[0].outfile:
						with open('AdditionalDomains.txt', 'a') as adom:
                                                        adom.writelines("%s\n" % item)
					if args[0].arcsight:	
						cef = 'CEF:0|IHC-OSINT|Soltra|1.0|100|Malicious '+'Domain'+'|1|request='+item+' msg=NH-ISAC Malicious Domain '+d.netloc
                                        	time.sleep(0.2)
                                        	syslog(cef, host=dest,port=dest_port)	
				else:
					if args[0].verbose:
					...
					if args[0].outfile:
						with open('Indicators.txt', 'a') as Idom:
                                                        Idom.writelines("%s\n" % item)
					if args[0].arcsight:
						cef = 'CEF:0|IHC-OSINT|Soltra|1.0|100|Known Malicious '+'Indicator'+'|1|cs1='+item+' msg=NH-ISAC Malicious Indicator '+item
                                        	time.sleep(0.2)
                                        	syslog(cef, host=dest,port=dest_port)
			except ValueError:
